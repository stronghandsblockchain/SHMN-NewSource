...
# Copyright (c) 2021 The SHMN developers
...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SHMNParams"
    else
        PARAMS_DIR="$HOME/.shmn-params"
    fi
fi
...
Failed to fetch the SHMN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SHMN - fetch-params.sh

This script will fetch the SHMN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SHMN zkSNARK parameters. Note that it is
